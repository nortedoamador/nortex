<x-app-layout title="{{ __('Cadastro de Embarcação') }}">
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="text-xl font-semibold tracking-tight text-slate-900 dark:text-white">{{ __('Cadastro de Embarcação') }}</h2>
        ...
        <div class="mx-auto max-w-[1000px]">
            <div class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                <form method="POST" action="{{ route('embarcacoes.store') }}" enctype="multipart/form-data" class="grid gap-4">
                    @csrf
                    @include('embarcacoes.partials.form-cadastro-campos', ['clientes' => $clientes, 'idPrefix' => ''])

                    <div class="flex items-center justify-end gap-3 pt-2">
                        <a href="{{ route('embarcacoes.index') }}" class="inline-flex items-center rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800">
                            {{ __('Cancelar') }}
                        </a>
