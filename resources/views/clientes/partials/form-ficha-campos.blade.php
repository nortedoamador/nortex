...
@if ($layout === 'stacked_sections')
    <div class="space-y-4">
        <div>
            <h3 class="text-sm font-semibold text-slate-900 dark:text-white">{{ __('Dados') }}</h3>
        </div>
        ...
        <div class="rounded-xl border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-700 dark:bg-slate-900/50">
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div
                    class="md:col-span-2 grid grid-cols-1 gap-6 @if ($tipoDoc !== 'pj') md:grid-cols-3 @endif"
                    data-cliente-doc-org-row
                >
                    <div>
                        <x-input-label
                            for="cpf"
                        ...
                        <x-text-input
                            ...
                            data-msg-cnpj-invalid="{{ __('CNPJ inválido.') }}"
                            placeholder="000.000.000-00"
                        />
                        <x-input-error :messages="$errors->get('cpf')" class="mt-2" />
                        <ul
                            class="mt-2 hidden list-none space-y-1 text-sm text-red-600 dark:text-red-400"
                            data-cliente-doc-principal-inline
                            role="alert"
                            aria-live="polite"
                        ></ul>
                    </div>
                    ...
                    <div data-doc-identidade-bloco="1">
                        ...
                        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('Órgão emissor do documento (CNH, RG ou CIN).') }}</p>
                        <x-input-error :messages="$errors->get('orgao_emissor')" class="mt-2" />
                    </div>
                </div>

                <div data-pf-only="1" class="md:col-span-2 {{ $tipoDoc === 'pj' ? 'hidden' : '' }}">
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                        <div>
                            ...
                            <p class="mt-1 text-xs leading-snug text-slate-500 dark:text-slate-400">{{ __('Este campo + emissão (dd/mm/aaaa) gera validade.') }}</p>
                        </div>
                        <div data-emissao-doc-wrap="1" class="{{ ($docIdTipo ?? '') === 'rg' ? 'md:col-span-2' : '' }}">
                            ...
                            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('Datas do documento informado (CNH, RG ou CIN).') }}</p>
                        </div>
                        <div data-validade-cnh-wrap="1" class="{{ ($docIdTipo ?? '') === 'rg' ? 'hidden' : '' }}">
                            <x-input-label for="validade_cnh" class="font-semibold text-slate-800 dark:text-slate-200">
                                {{ __('Validade (CIN e CNH)') }}
                            </x-input-label>
                            <input
                                id="validade_cnh"
                                name="validade_cnh"
                ...
                <div class="md:col-span-2 grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div data-pf-only="1" class="{{ $tipoDoc === 'pj' ? 'hidden' : '' }}">
