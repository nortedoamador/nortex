<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-semibold text-slate-900 dark:text-white">{{ __('Cadastro de Clientes') }}</h2>
                <p class="text-sm text-slate-600 dark:text-slate-400">{{ __('Ficha de cadastro') }}</p>
            </div>
            <a href="{{ route('clientes.index') }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400">{{ __('← Clientes') }}</a>
        </div>
    </x-slot>
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-8">
                ...
                >
                    ...
                    <div class="flex flex-wrap gap-3 border-t border-slate-200 pt-6 dark:border-slate-700">
                        ...
                        <a href="{{ route('clientes.index') }}" class="inline-flex items-center rounded-md border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 shadow-sm dark:border-slate-600 dark:bg-slate-900 dark:text-slate-200">{{ __('Cancelar') }}</a>
                    </div>
                </form>
            </div>
        </div>
