<x-tenant-admin-layout title="{{ __('Editar tipo') }}">
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-slate-900 dark:text-white">{{ __('Editar tipo de processo') }}</h2>
    </x-slot>

    <div class="px-4 py-6 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-xl">
            <form method="POST" action="{{ tenant_admin_route('tipo-processos.update', $tipoProcesso) }}" class="space-y-4">
                @csrf
                @method('PATCH')
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 space-y-4">
                    <div>
                        ...
                        <input name="nome" value="{{ old('nome', $tipoProcesso->nome) }}" required class="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                    ...
                    <div>
                        ...
                        <input name="slug" value="{{ old('slug', $tipoProcesso->slug) }}" required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" class="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2 font-mono text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                    ...
                    <div>
                        ...
                        <select name="categoria" required class="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white">
                            @foreach ($categorias as $c)
                                <option value="{{ $c->value }}" @selected(old('categoria', $tipoProcesso->categoria->value) === $c->value)>{{ $c->label() }}</option>
                            @endforeach
                        </select>
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <a href="{{ tenant_admin_route('tipo-processos.edit-regras', $tipoProcesso) }}" class="rounded-lg border border-violet-300 px-5 py-2.5 text-sm font-semibold text-violet-700 dark:border-violet-800 dark:text-violet-300">{{ __('Checklist') }}</a>
