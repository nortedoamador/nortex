<x-tenant-admin-layout title="{{ __('Tipos de documento') }}">
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="text-xl font-semibold text-slate-900 dark:text-white">{{ __('Tipos de documento') }}</h2>
            <a href="{{ tenant_admin_route('documento-tipos.create') }}" class="inline-flex items-center rounded-lg bg-indigo-600 px-4 py-2 text-xs font-semibold uppercase tracking-widest text-white transition hover:bg-indigo-500">
                {{ __('Novo tipo') }}
            </a>
        </div>
    </x-slot>
                        ...
                        @foreach ($tipos as $t)
                            <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                                ...
                                <td class="whitespace-nowrap px-4 py-3 text-right text-sm">
                                    <a href="{{ tenant_admin_route('documento-tipos.edit', $t) }}" class="font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400">{{ __('Editar') }}</a>
                                    <form method="POST" action="{{ tenant_admin_route('documento-tipos.destroy', $t) }}" class="ms-3 inline" onsubmit="return confirm(@json(__('Tem certeza?')));">
                                        @csrf
                                        @method('DELETE')
