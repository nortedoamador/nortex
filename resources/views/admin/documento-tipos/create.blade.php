<x-tenant-admin-layout title="{{ __('Novo tipo de documento') }}">
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-slate-900 dark:text-white">{{ __('Novo tipo de documento') }}</h2>
    </x-slot>

    <div class="px-4 py-6 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-xl">
            <form method="POST" action="{{ tenant_admin_route('documento-tipos.store') }}" class="space-y-4">
                @csrf
                <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 space-y-4">
                    ...
                    <div>
                        <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
                            <input type="checkbox" name="auto_gerado" value="1" @checked(old('auto_gerado')) class="rounded border-slate-300 text-indigo-600" />
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ tenant_admin_route('documento-tipos.index') }}" class="rounded-lg border border-slate-300 px-5 py-2.5 text-sm font-semibold dark:border-slate-600">{{ __('Cancelar') }}</a>
                </div>
            </form>
        </div>
    </div>
