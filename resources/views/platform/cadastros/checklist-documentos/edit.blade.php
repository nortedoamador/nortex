<x-platform-layout :title="__('Editar tipo de documento (global)')">
    <x-slot name="header">
        <div>
            <h2 class="text-xl font-semibold text-slate-900 dark:text-white">{{ __('Editar tipo de documento (global)') }}</h2>
    ...
    <div class="mx-auto max-w-xl">
        <form method="POST" action="{{ route('platform.cadastros.checklist-documentos.update', $documentoTipo->id) }}" class="space-y-4">
            @csrf
            @method('PATCH')
            <div class="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                ...
                <div>
                    <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
                        <input type="checkbox" name="auto_gerado" value="1" @checked(old('auto_gerado', $documentoTipo->auto_gerado)) class="rounded border-slate-300 text-violet-600" />
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('platform.cadastros.checklist-documentos.index') }}" class="rounded-lg border border-slate-300 px-5 py-2.5 text-sm font-semibold dark:border-slate-600">{{ __('Voltar') }}</a>
            </div>
        </form>
    </div>
</x-platform-layout>
