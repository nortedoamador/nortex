<x-platform-layout :title="__('Editar tipo de anexo')">
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-slate-900 dark:text-white">{{ __('Editar tipo de anexo') }} — {{ $tipo->nome }}</h2>
    </x-slot>

    <div class="max-w-xl space-y-4">
        <form method="POST" action="{{ route('platform.cadastros.anexo-tipos.update', $tipo) }}" class="space-y-4">
            @csrf
            @method('PATCH')
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" min="1" max="2048" name="max_size_mb" value="{{ old('max_size_mb', $tipo->max_size_mb) }}" required class="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" />
                        @error('max_size_mb')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    <div class="flex items-end">
                        <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
                            <input type="hidden" name="is_multiple" value="0" />
                            <input type="checkbox" name="is_multiple" value="1" @checked(old('is_multiple', $tipo->is_multiple)) class="rounded border-slate-300 text-violet-600" />
                ...
                <div>
                    ...
                    <textarea name="allowed_mime_types" rows="3" class="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" placeholder="application/pdf, image/jpeg">{{ old('allowed_mime_types', is_array($tipo->allowed_mime_types) ? implode(', ', $tipo->allowed_mime_types) : '') }}</textarea>
                ...
                <div>
                    ...
                    <textarea name="allowed_extensions" rows="2" class="mt-1 w-full rounded-xl border border-slate-200 px-4 py-2 text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-white" placeholder="pdf, jpg, png">{{ old('allowed_extensions', is_array($tipo->allowed_extensions) ? implode(', ', $tipo->allowed_extensions) : '') }}</textarea>
                    <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('Separe por vírgula ou por linha. Em branco = qualquer.') }}</p>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('platform.cadastros.anexo-tipos.index') }}" class="rounded-lg border border-slate-300 px-5 py-2.5 text-sm font-semibold dark:border-slate-600">{{ __('Voltar') }}</a>
            </div>
        </form>
    </div>
</x-platform-layout>
