<x-platform-layout :title="__('Editar tipo de serviço')">
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-slate-900 dark:text-white">{{ __('Editar tipo de serviço') }} — {{ $tipo->nome }}</h2>
    </x-slot>

    <div class="max-w-xl space-y-4">
        <form method="POST" action="{{ route('platform.cadastros.tipos-servico.update', $tipo) }}" class="space-y-4">
            @csrf
            @method('PATCH')
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('platform.cadastros.tipos-servico.index') }}" class="rounded-lg border border-slate-300 px-5 py-2.5 text-sm font-semibold dark:border-slate-600">{{ __('Voltar') }}</a>
            </div>
        </form>
    </div>
</x-platform-layout>
