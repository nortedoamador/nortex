<x-platform-layout :title="__('Novo tipo de processo')">
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-slate-900 dark:text-white">{{ __('Novo tipo de processo') }}</h2>
    </x-slot>

    <div class="max-w-xl space-y-4">
        <form method="POST" action="{{ route('platform.cadastros.tipos-processo.store') }}" class="space-y-4">
            @csrf
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('platform.cadastros.tipos-processo.index') }}" class="rounded-lg border border-slate-300 px-5 py-2.5 text-sm font-semibold dark:border-slate-600">{{ __('Cancelar') }}</a>
            </div>
        </form>
    </div>
</x-platform-layout>
