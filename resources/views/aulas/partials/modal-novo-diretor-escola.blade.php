...
<x-modal name="novo-diretor-escola" maxWidth="5xl" focusable>
    <div class="flex max-h-[min(92vh,920px)] flex-col overflow-hidden bg-white dark:bg-slate-900">
        <div class="flex shrink-0 items-start justify-between gap-3 border-b border-slate-200 px-5 py-4 dark:border-slate-700 sm:px-6">
            <div>
                <h2 class="text-lg font-semibold text-slate-900 dark:text-white">{{ __('Cadastrar diretor') }}</h2>
                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('Os dados seguem a ficha de cadastro de clientes e ficam associados à escola ao guardar o perfil.') }}</p>
            </div>
            <button type="button" class="rounded-lg p-2 text-slate-500 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800" @click="$dispatch('close-modal', 'novo-diretor-escola')">
        ...
        <div class="min-h-0 flex-1 overflow-y-auto px-5 py-4 sm:px-6 sm:py-5">
            <form
                ...
                x-data="nxNovoDiretorEscolaForm()"
            ...
            >
                ...
                <div class="sticky bottom-0 border-t border-slate-200 bg-white pt-4 dark:border-slate-700 dark:bg-slate-900">
                    <div class="flex flex-wrap items-center justify-end gap-2">
                        <button type="button" class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200" @click="$dispatch('close-modal', 'novo-diretor-escola')">{{ __('Cancelar') }}</button>
