    ...
    <x-escola-hub-frame>
        ...
        <div class="mx-auto max-w-5xl space-y-4">
            ...
            <div class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    @if (auth()->user()?->hasPermission('aulas.view'))
                        <div class="flex flex-wrap gap-2">
                            ...
                            <a class="inline-flex items-center rounded-lg bg-slate-900 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white" href="{{ route('aulas.pdf.ara', $aula) }}" data-turbo="false" target="_blank" rel="noopener noreferrer">{{ __('ARA (lote)') }}</a>
                ...
                <p class="mt-2 text-xs text-slate-500 dark:text-slate-400">{{ __('Modelos oficiais serão aplicados quando fornecidos; por ora, o PDF é um placeholder funcional.') }}</p>
            </div>
