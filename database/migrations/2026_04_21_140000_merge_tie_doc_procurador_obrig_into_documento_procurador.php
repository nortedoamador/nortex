...
/**
 * Funde TIE_DOC_PROCURADOR_OBRIG no tipo único DOCUMENTO_PROCURADOR (mesmo nome de catálogo).
 */
return new class extends Migration
{
     ...
     * se já existir linha com documento_tipo_id = destino, funde obrigatorio/ordem e apaga a origem.
     */
    ...
    /** Evita violar proc_doc_unique (processo_id + documento_tipo_id). */
    ...
    private function dedupeDocumentoProcessoPivot(): void
    {
        ...
        $rows = DB::table('documento_processo')->orderBy('id')->get([
            'id',
            'empresa_id',
        ...
        foreach ($buckets as $group) {
            ...
            $keep = $group[0];
            $mergeObrigatorio = false;
            $minOrdem = PHP_INT_MAX;
            foreach ($group as $r) {
                $mergeObrigatorio = $mergeObrigatorio || (bool) $r->obrigatorio;
                $minOrdem = min($minOrdem, (int) ($r->ordem ?? 0));
            }
            $keepId = (int) $keep->id;

            DB::table('documento_processo')->where('id', $keepId)->update([
                'obrigatorio' => $mergeObrigatorio,
                'ordem' => $minOrdem,
                'updated_at' => now(),
            ]);
            ...
            foreach ($group as $r) {
                ...
                DB::table('documento_processo')->where('id', (int) $r->id)->delete();
            }
        }
    }
