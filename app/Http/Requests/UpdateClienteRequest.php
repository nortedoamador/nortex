...
class UpdateClienteRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $docTipo = (string) ($this->input('documento_identidade_tipo') ?: ($cliente->documento_identidade_tipo ?: BrasilOrgaoEmissorDocumento::TIPO_CNH));
        $orgaos = BrasilOrgaoEmissorDocumento::valoresPermitidosPara($docTipo);
        if (filled($cliente->orgao_emissor) && ! in_array($cliente->orgao_emissor, $orgaos, true)) {
            $orgaos[] = $cliente->orgao_emissor;
        }

        $nacionalidades = NacionalidadesComuns::valoresPermitidos();
        if (filled($cliente->nacionalidade) && ! in_array($cliente->nacionalidade, $nacionalidades, true)) {
            $nacionalidades[] = $cliente->nacionalidade;
        }
